...
CREATE PROCEDURE [dbo].[spReecritureImportInfo_fichiers_XML_nouveau]
	--Test Run: [dbo].[spReecritureImportInfo_fichiers_XML_nouveau] 5,'Ithaque App','EC85_output_85008_1793-1800_NMD_260.xml,EC85_output_85008_1843-1848_NMD_283.xml,EC85_output_85136_1793-1794_N_4288.xml,EC85_output_85140_1812-1826_NMD_4360.xml'
...
BEGIN
	...
	SELECT dxf2.id_fichier, x.date_creation, x.nom_fichier
	FROM
	(
	SELECT MIN(dxf.date_creation) as date_creation,dxf.nom_fichier
	...
	--WHERE de.traite=1 AND t.livraison_annulee=0 AND ISNULL(dxf.rejete, 0) = 0 AND ISNULL(dxf.ignore_export_publication, 0) = 0  --  and de.id_statut in (1,3)
	GROUP BY dxf.nom_fichier) as x
	INNER JOIN [DataXml_fichiers] dxf2 with (nolock) on x.date_creation = dxf2.date_creation 
					AND x.nom_fichier = dxf2.nom_fichier
	...
	BEGIN TRANSACTION spReecritureImportInfo;  
	   ...
	   truncate table Ithaque_Reecriture.dbo.DataXmlRC_actes
		...
		SELECT [id],[type_acte],[soustype_acte],[id_acte_xml],[date_acte],[annee_acte],[calendrier_date_acte],[lieu_acte]
		  ...
		  ,[date_acte_Correction],[annee_acte_Correction],[calendrier_date_acte_Correction],[lieu_acte_Correction],[geonameid_acte_Correction],[insee_acte_Correction]
		  ,[id_fichier],[type_acte_Correction],[id_image],[date_acte_Correction_Reecriture],[annee_acte_Correction_Reecriture]
		FROM (SELECT ROW_NUMBER() OVER (PARTITION BY dxa.id_acte_xml ORDER BY dxa.id ASC) AS line,dxa.[id],[type_acte],[soustype_acte],[id_acte_xml],[date_acte],[annee_acte],[calendrier_date_acte],[lieu_acte],[geonameid_acte],[insee_acte],[fichier_xml_acte]
		  ...
		  ,[type_acte_Correction],[id_image],[date_acte_Correction_Reecriture],[annee_acte_Correction_Reecriture] 
			  ...
			  INNER JOIN Ithaque.[dbo].DataXml_fichiers dxf WITH (NOLOCK)ON dxa.id_fichier = dxf.id_fichier
			  INNER JOIN Ithaque.[dbo].Traitements t WITH (NOLOCK) ON dxf.id_traitement = t.id_traitement
			  INNER JOIN #temp_ID_fichier_test1 tf ON tf.id_fichier=dxf.id_fichier) as dtt
		WHERE line=1
