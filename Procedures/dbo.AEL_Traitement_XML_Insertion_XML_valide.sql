...
-- =============================================
CREATE PROCEDURE [dbo].[AEL_Traitement_XML_Insertion_XML_valide]
...
END
	...
	--	,'ANI', 'ANII', 'ANIII', 'ANIV','ANV', 'ANVI', 'ANVII', 'ANVIII', 'ANIX', 'ANX', 'ANXI', 'ANXII', 'ANXIII', 'ANXIV')
	--	OR isnull(t.annee_acte , '') = '' )
	--	AND '\' + REVERSE(SUBSTRING(REVERSE(chemin_fichier),0,CHARINDEX('\',REVERSE(chemin_fichier)))) collate database_default
	--		NOT IN (SELECT chemin FROM [PRESQL04].Archives_Preprod.dbo.saisie_donnees_fichiers_xml_temp WITH (NOLOCK))
	--GROUP BY '\' + REVERSE(SUBSTRING(REVERSE(chemin_fichier),0,CHARINDEX('\',REVERSE(chemin_fichier))))
GO
