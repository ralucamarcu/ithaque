...
-- =============================================
CREATE PROCEDURE [dbo].[AEL_Insertion_Parametres_MAJ_MEP]
...
BEGIN
    ...
    UPDATE dbo.AEL_Parametres_MAJ_MEP
	SET principal = 0	
	WHERE principal=1
	
	IF NOT EXISTS (SELECT * FROM AEL_Parametres_MAJ_MEP WHERE id_projet = @id_projet AND chemin_dossier_general = @chemin_dossier_general)
	BEGIN		
		INSERT INTO dbo.AEL_Parametres_MAJ_MEP([titre_document],[description],[miniature_document],[sous_titre],[image_principale],[image_secondaire]
		  ,[publie],[date_publication],[id_projet],[chemin_dossier_v4],[chemin_dossier_v5],[chemin_xml],[id_association],[date_creation_projet]
		  ,[chemin_dossier_v6],[principal],[chemin_dossier_general], id_livraisons,id_document)
		...
		,@chemin_dossier_v6,1,@chemin_dossier_general, @id_livraisons,@id_document)
	END
	ELSE
	BEGIN
		SET @id_ael_parametre = (SELECT id_ael_parametres FROM AEL_Parametres_MAJ_MEP WHERE id_projet = @id_projet AND chemin_dossier_general = @chemin_dossier_general)
		UPDATE dbo.AEL_Parametres_MAJ_MEP
