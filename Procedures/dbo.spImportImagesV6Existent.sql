...
-- =============================================
CREATE PROCEDURE [dbo].[spImportImagesV6] 
	-- Add the parameters for the stored procedure here
	-- Test Run : [spImportImagesV6] '29C1BDE7-D8AE-428B-BCB6-2BD0D79BA0BD', 2, null, 4,5, '\\10.1.0.196\bigvol\Projets\p37-AD85\V4\images\','\\10.1.0.196\bigvol\Projets\p37-AD85\V5\images\'
...
BEGIN
    ...
    DECLARE @date DATETIME = GETDATE(), @id_image UNIQUEIDENTIFIER, @id_tache_log INT, @IntegerResult INT, @id_version_v6 INT
    SET @id_version_v6 = (SELECT id_status_version_processus FROM Taches WITH (NOLOCK) WHERE id_tache = @id_tache_v6)
    
	CREATE TABLE #tmpFiles([ID] [INT] NULL,[FILE_PATH] [VARCHAR](500) NULL,[FILE_NAME] [VARCHAR](128) NULL)
    INSERT INTO #tmpFiles
    SELECT * FROM Files WITH (NOLOCK) WHERE file_path LIKE '%V6%'
	...
	SET @id_tache_log = (SELECT TOP 1 id_tache_log FROM dbo.Taches_Logs WITH (NOLOCK)
					WHERE id_projet = @id_projet AND id_utilisateur = @id_utilisateur AND id_tache = @id_tache_v6				
					ORDER BY date_creation DESC)
	--PRINT @id_tache_log 
	
	BEGIN TRY
	BEGIN TRANSACTION spImportImagesV6;
			...
			SELECT tli.id_image, tli.path_destination AS path_origine, t.FILE_PATH AS path_destination, @id_tache_v6 as id_tache
			...
			INNER JOIN #tmpFiles t WITH (NOLOCK) 
				ON REPLACE(tli.path_destination, @v5_general_path, '') = REPLACE(t.FILE_PATH, @v6_general_path,'')
				AND tli.nom_fichier_image_destination=t.[FILE_NAME] 	
			
			UPDATE Images 
			SET id_projet = @id_projet
				, id_status_version_processus = @id_version_v6
			FROM Images i WITH (NOLOCK)
			INNER JOIN Taches_Images_Logs tli WITH (NOLOCK) ON i.id_image = tli.id_image
			WHERE id_tache = @id_tache_v6 and id_source = @id_source
			
	DROP TABLE #tmpFiles
			
	COMMIT TRANSACTION spImportImagesV6;
	...
	BEGIN CATCH
		...
		ROLLBACK TRANSACTION spImportImagesV6;
