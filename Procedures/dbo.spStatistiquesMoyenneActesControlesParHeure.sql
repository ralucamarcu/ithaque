...
-- =============================================
CREATE PROCEDURE [dbo].[spStatistiquesMoyenneActesControlesParHeure] 
	-- Add the parameters for the stored procedure here	
	 @date_fin DATETIME = NULL
	,@statistique_type INT = 1
AS
BEGIN
	...
	IF @date_fin IS NULL
		SET @date_fin = DATEADD(DAY, -1, GETDATE())
...
-- Statistiques fait globalement pour tous les utilisateur
	...
	WHERE id_echantillon IN (SELECT x.id_objet FROM (SELECT id_objet, DATEADD(dd,0,DATEDIFF(dd,0,date_log)) AS date_log
						...
						HAVING DATEADD(dd,0,DATEDIFF(dd,0,date_log)) >= @date_debut AND DATEADD(dd,0,DATEDIFF(dd,0,date_log)) < @date_fin) AS x )
