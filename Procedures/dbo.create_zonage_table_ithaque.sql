...
--testrun: [create_zonage_table_ithaque] '9219D296-1BEB-4E37-9ABF-A71B592B2421'
CREATE PROCEDURE [dbo].[create_zonage_table_ithaque] 
...
PRINT 1
	...
	select x.id_fichier
	from (
	select ROW_NUMBER() over(partition by dxf.nom_fichier order by dxf.date_creation desc)as row_num, dxf.id_fichier  
	from dataxml_fichiers dxf with (nolock)
	inner join traitements t with (nolock) on dxf.id_traitement = t.id_traitement
	where t.id_projet = @id_projet) as x
	where x.row_num = 1
...
PRINT 4
 ...
 PRINT 5
	...
	IF (@nom_projet_dossier LIKE '%-RC')
	BEGIN
		set @command = '"C:\Program Files\Microsoft SQL Server\100\Tools\Binn\bcp.exe" [Ithaque].[dbo].[tmp_zonage] out \\10.1.0.196\bigvol\Jobs\zonage_fichiers_recensement\' 
		+ @nom_fichier + '.txt -T -E -SARDSQL02\ARDSQL02FR -t -c'
	END
	ELSE
	BEGIN
	 set @command = '"C:\Program Files\Microsoft SQL Server\100\Tools\Binn\bcp.exe" [Ithaque].[dbo].[tmp_zonage] out \\10.1.0.196\bigvol\Jobs\zonage_fichiers\' 
		+ @nom_fichier + '.txt -T -E -SARDSQL02\ARDSQL02FR -t -c'
	END
...
PRINT 6
	...
	insert into #tmp		
	exec xp_dirtree '\\10.1.0.196\bigvol\Jobs\zonage_fichiers',1,1
