...
-- =============================================
CREATE PROCEDURE [dbo].[spStatistiquesControleQualite_v1]
  ...
  -- 	IF OBJECT_ID('Statistiques_Controle_Qualite_Operateurs') IS NOT NULL 
	...
	BEGIN
		...
		SET @actes_lus = (SELECT COUNT(DISTINCT dxa.id) 
			FROM DataXmlRC_actes dxa WITH (NOLOCK)
			INNER JOIN DataXmlEchantillonDetailsRC dxed WITH (NOLOCK) ON dxa.id = dxed.id_acte_RC
			INNER JOIN DataXmlEchantillonEntete dxee WITH (NOLOCK) ON dxed.id_echantillon = dxee.id_echantillon
			INNER JOIN #tmp_lots_controles t ON dxee.id_lot = t.id_lot
			WHERE dxed.traite = 1)
	
		TRUNCATE TABLE #tmp_lots_controles
	
				
		INSERT INTO temp4 (id_utilisateur, operateur_nom, operateur_prenom, [DATE]
