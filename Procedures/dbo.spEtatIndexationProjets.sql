...
-- Description:	Renvoie les nombres des fichiers et des actes livrés, à contrôler, acceptés, rejetés... par projet
-- =============================================
CREATE PROCEDURE [dbo].[spEtatIndexationProjets]
AS
BEGIN
	...
	--Projets retraités (A')
	IF OBJECT_ID('dbo.#temp_projets_retraites') IS NOT NULL
		drop table #temp_projets_retraites
	select *, 'A''' as statut
	into #temp_projets_retraites
	from Projets (nolock)
	where id_projet in ('9219D296-1BEB-4E37-9ABF-A71B592B2421', 'D1A682D4-D117-4D2C-A256-9855EF79DEAC', '9DDEBB0E-38D1-48A7-85E1-F561F2B95AA1','0C4F8E64-047A-4552-8725-929E205B2EE2','E2AC10C8-B670-4F9A-8DFD-1CE471207FBF','B0F8B9B0-99E2-4A12-BB18-E021D0C7F562','3A7C7386-EACF-4BBE-BDFA-39F77659FA01','4CE67ECB-B269-44C8-927B-52B354F808DB','B4323FCA-F824-42F7-8116-630E2EC0F10F','F34FEEC1-0BB3-4D21-A356-EDB44A365648','7DB302ED-8525-48B7-8CEC-932FCEBB27D8','B48730B4-8B2D-4AEF-8EDB-9D8D844AA2A0','EB3FE005-3836-4AEE-BF4A-B76C5406ED8B','5687DAFC-85C7-4D9B-957A-EBBA04F7ACAC','E1E95870-9881-41C9-AD00-B112DA373A7A','0C13983F-845D-4DEF-891E-12F382D347EE','F08A87A0-73C6-43FD-BCC8-2E1637BBE991','BC222059-010B-45C8-85BF-DD246BC4F41B','0F49F5DA-800F-465F-A27E-39C16F3C93B3','EA12F801-0305-409F-A526-8E7FC0954B6F','023B3053-73CD-4990-9B48-5522EBD59C29','A89FAAEC-57DC-42DD-A1EC-D3303910AF96','FC645973-F83D-4D52-9A56-72764D4F92B8','C94C3D9C-1F04-4629-934F-5EFDCD782768','8F80C9ED-6A2E-4B62-B955-6C222B3B4C63','5EB600EE-FF22-440B-85B5-4678CC3993D7')
		or id_projet in ('51e03f94-ff6b-4fd4-83dc-a776b8b0a35e','1376e61f-e580-49eb-86c0-c3e9295f8cab')
	--Projets non retraités (A)
	IF OBJECT_ID('dbo.#temp_projets_non_retraites') IS NOT NULL
		drop table #temp_projets_non_retraites
	select *, 'A' as statut
	into #temp_projets_non_retraites
	from Projets (nolock)
	where id_projet_maitre in ('43DCD200-3D23-47DE-BE93-5A795B2922B1', '1DEE67D4-0EA9-4B0D-94B5-584B7279B1F0') and id_projet not in (select id_projet from #temp_projets_retraites) and id_projet not in ('65042153-90E9-47E8-9BDA-C0DBE370EDD2', '5BC4606A-A0F3-46A9-9542-91D457F271E7', '6F8401B1-301E-44B9-B895-737183B56401', '18F9F73B-2F20-4E11-9396-CE3208715190','2BA5B255-9C49-4F87-AB72-F4D95E9C93A4','09302A58-FAA0-4441-B1D0-D9683BFBEA94','8C6D2075-B353-45E3-96B3-821E4D349BB6','0B86DAC0-599A-4A48-8B16-7E79792779C7','80FC9ADF-A611-4B47-95BB-E7CF395ABEE0','FD653D02-BF44-4678-A243-1B5DC34F8BEC')

	--Tous les projets
	IF OBJECT_ID('dbo.#temp_projets') IS NOT NULL
		drop table #temp_projets
	select * into #temp_projets from 
	(
	select *
	...
	IF OBJECT_ID('dbo.#nb_xml_attendus_projet') IS NOT NULL
		drop table #nb_xml_attendus_projet
	select p.id_projet, COUNT(fd.id_fichier) as [nb_xml_attendus]
	into #nb_xml_attendus_projet
	from FichiersData fd (nolock)
	inner join #temp_projets p (nolock) on fd.id_projet=p.id_projet
	group by p.id_projet

	--Tous les fichiers acceptés de tous les projets
	IF OBJECT_ID('dbo.#temp_fichiers_acceptes') IS NOT NULL
		drop table #temp_fichiers_acceptes
	select p.id_projet, p.nom as projet, t.nom as traitement, f.*
	into #temp_fichiers_acceptes
	...
	inner join #temp_projets p (nolock) on t.id_projet=p.id_projet --and p.id_projet='9219D296-1BEB-4E37-9ABF-A71B592B2421'
	...
	IF OBJECT_ID('dbo.#temp_projets_non_retraites') IS NOT NULL
		drop table #temp_fichiers_a_controler
	select p.id_projet, p.nom as projet, t.nom as traitement, f.*
	into #temp_fichiers_a_controler
	from dbo.DataXml_fichiers f (nolock)
	inner join Traitements t (nolock) on f.id_traitement=t.id_traitement
	inner join #temp_projets p (nolock) on t.id_projet=p.id_projet --and p.id_projet='7DB302ED-8525-48B7-8CEC-932FCEBB27D8'
	...
	IF OBJECT_ID('dbo.#temp_fichiers_rejetes') IS NOT NULL
		drop table #temp_fichiers_rejetes
	/*select p.id_projet, p.nom as projet, t.nom as traitement, f.*
	into #temp_fichiers_rejetes
	from dbo.DataXml_fichiers f (nolock)
	inner join Traitements t (nolock) on f.id_traitement=t.id_traitement
	inner join #temp_projets p (nolock) on t.id_projet=p.id_projet --and p.id_projet='9219D296-1BEB-4E37-9ABF-A71B592B2421'
	inner join dbo.DataXml_lots_fichiers lf (nolock) on t.id_traitement=lf.id_traitement and f.id_lot=lf.id_lot
	...
	order by p.nom, f.nom_fichier*/
	select * into #temp_fichiers_rejetes from (
	...
	from dbo.DataXml_fichiers f (nolock)
	inner join Traitements t (nolock) on f.id_traitement=t.id_traitement
	inner join #temp_projets p (nolock) on t.id_projet=p.id_projet
	inner join dbo.DataXml_lots_fichiers lf (nolock) on t.id_traitement=lf.id_traitement and f.id_lot=lf.id_lot
	inner join dbo.DataXmlEchantillonEntete ee (nolock) on ee.id_lot=lf.id_lot and ee.traite=1 and ee.id_statut in (2)
	left join #temp_fichiers_acceptes fa on fa.id_fichierData=f.id_fichierData
	left join #temp_fichiers_a_controler fac on fac.id_fichierData=f.id_fichierData
	...
	IF OBJECT_ID('dbo.#temp_fichiers_recus_jamais_controles') IS NOT NULL
		drop table #temp_fichiers_recus_jamais_controles
	select p.id_projet, p.nom as projet, t.nom as traitement, f.*
	into #temp_fichiers_recus_jamais_controles
	from dbo.DataXml_fichiers f (nolock)
	inner join Traitements t (nolock) on f.id_traitement=t.id_traitement
	inner join #temp_projets p (nolock) on t.id_projet=p.id_projet --and p.id_projet='7DB302ED-8525-48B7-8CEC-932FCEBB27D8'
	left join dbo.DataXml_lots_fichiers lf (nolock) on t.id_traitement=lf.id_traitement and f.id_lot=lf.id_lot
	...
	--select projet, id_fichier, count(id_fichier) from #temp_fichiers_rejetes group by projet, id_fichier /*having count(id_fichier)>1*/ order by projet, id_fichier
	--select * from #temp_fichiers_acceptes where id_fichier='950DDD69-E7F2-4D3F-A0D0-0002EE504EE6'
	IF OBJECT_ID('dbo.#temp_fichiers_acceptes_nb') IS NOT NULL
		drop table #temp_fichiers_acceptes_nb
	select id_projet, projet, COUNT(f.id_fichier) as nb_fichiers, COUNT(distinct f.id_fichierData) as nb_fichierData, SUM(f.nb_actes_naissance)+SUM(f.nb_actes_mariage)+SUM(f.nb_actes_deces) as [nb_actes_NMD], SUM(f.nb_actes) as [nb_actes] into #temp_fichiers_acceptes_nb from #temp_fichiers_acceptes f group by id_projet, projet order by projet
	IF OBJECT_ID('dbo.#temp_fichiers_rejetes_nb') IS NOT NULL
		drop table #temp_fichiers_rejetes_nb
	select id_projet, projet, COUNT(f.id_fichier) as nb_fichiers, COUNT(distinct f.id_fichierData) as nb_fichierData, SUM(f.nb_actes_naissance)+SUM(f.nb_actes_mariage)+SUM(f.nb_actes_deces) as [nb_actes_NMD], SUM(f.nb_actes) as [nb_actes] into #temp_fichiers_rejetes_nb from #temp_fichiers_rejetes f group by id_projet, projet order by projet
	IF OBJECT_ID('dbo.#temp_fichiers_a_controler_nb') IS NOT NULL
		drop table #temp_fichiers_a_controler_nb
	select id_projet, projet, COUNT(f.id_fichier) as nb_fichiers, COUNT(distinct f.id_fichierData) as nb_fichierData, SUM(f.nb_actes_naissance)+SUM(f.nb_actes_mariage)+SUM(f.nb_actes_deces) as [nb_actes_NMD], SUM(f.nb_actes) as [nb_actes] into #temp_fichiers_a_controler_nb from #temp_fichiers_a_controler f group by id_projet, projet order by projet
	IF OBJECT_ID('dbo.#temp_fichiers_recus_jamais_controles_nb') IS NOT NULL
		drop table #temp_fichiers_recus_jamais_controles_nb
	select id_projet, projet, COUNT(f.id_fichier) as nb_fichiers, COUNT(distinct f.id_fichierData) as nb_fichierData, SUM(f.nb_actes_naissance)+SUM(f.nb_actes_mariage)+SUM(f.nb_actes_deces) as [nb_actes_NMD], SUM(f.nb_actes) as [nb_actes] into #temp_fichiers_recus_jamais_controles_nb from #temp_fichiers_recus_jamais_controles f group by id_projet, projet order by projet
	/*
	...
	*/
	...
	select p.id_projet, p.nom as projet, p.statut, CASE WHEN p.indexation_xml_terminee=1 THEN 'Oui' ELSE '' END as [indexation terminée], z.nb_xml_attendus
	...
	/*
	select * from #temp_fichiers_rejetes where id_projet='3BAACDFB-0F61-4A18-BD34-AC7FEFCC6E60' --and id_traitement=557
	select * from (
	select ROW_NUMBER() OVER(PARTITION BY f.id_FichierData ORDER BY f.id_traitement DESC) as rn, p.id_projet, p.nom as projet, t.nom as traitement, f.*
	from dbo.DataXml_fichiers f (nolock)
	inner join Traitements t (nolock) on f.id_traitement=t.id_traitement
	inner join #temp_projets p (nolock) on t.id_projet=p.id_projet and p.id_projet='3BAACDFB-0F61-4A18-BD34-AC7FEFCC6E60'
	inner join dbo.DataXml_lots_fichiers lf (nolock) on t.id_traitement=lf.id_traitement and f.id_lot=lf.id_lot
	inner join dbo.DataXmlEchantillonEntete ee (nolock) on ee.id_lot=lf.id_lot and ee.traite=1 and ee.id_statut in (2)
	left join #temp_fichiers_acceptes fa on fa.id_fichierData=f.id_fichierData
	left join #temp_fichiers_a_controler fac on fac.id_fichierData=f.id_fichierData
